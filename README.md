...
## problems solved by Maven
  ...
  - manually add JAR files to our build path/classpath
  - We tell Maven the dependencies we are working on.
  - maven will go and download the JAR files of these projects and will make those available during compile/run

## How maven works
  - we have a config file in the project which Maven reads
  - check maven local repo if the dependencies are present if not will pull it from Maven central repo (remote)
  - maven will also download supporting dependencies

# pom.xl
  - Project Object Model file(pom file)
  - configuration file of the project
  - maven uses this to get the dependencies in project
  - located in the root
...
# Application properties
  - all properties for spring are picked up from this file
  - file path is `src/main/resources/application.properties`
  - it will be created by the spring initializer
